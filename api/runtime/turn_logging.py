...
def print_turn_summary(
    *,
    ...
    if config.LOG_CONVERSATION_CONTENT:
        ...
        print("AI_CONVO | content_logging: disabled", flush=True)
    print(
        "AI_CONVO | "
    ...
    print(
        ...
        f"actions={action_count}"
        f"{_content_summary(transcript, response_text)}",
        flush=True,
    )
...
def _content_summary(transcript: str, response_text: str) -> str:
    if not config.LOG_CONVERSATION_CONTENT:
        return ""
    return f' transcript="{_clip(transcript, 90)}" response="{_clip(response_text, 120)}"'
