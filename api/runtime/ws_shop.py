...
class WebSocketShopSession:
        ...
        try:
            while True:
                ...
                if message_type == WS_TYPE_AUDIO_CHUNK:
                    ...
                    if not await self.append_audio_chunk(_decode_audio_chunk(payload)):
                        continue
                    continue
    ...
    async def append_audio_chunk(self, chunk: bytes) -> bool:
        next_size = self.audio_size_bytes + len(chunk)
        if next_size > config.MAX_AUDIO_UPLOAD_BYTES:
            self.audio_chunks = []
            self.audio_size_bytes = 0
            await self.send({"type": WS_TYPE_ERROR, "message": "Audio payload is too large."})
            return False
        self.audio_chunks.append(chunk)
        self.audio_size_bytes = next_size
        return True
