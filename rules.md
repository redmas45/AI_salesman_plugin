...
* **Read Before Editing:** Read this file, `handoff.md`, relevant manifests, tests, and nearby implementation before changing code.
...
* **Ask, Never Assume:** When a constraint the turn depends on is missing or genuinely ambiguous, ask one short question. Do not substitute a value the user did not say — not from the retrieved records, not from the current page, not from an earlier topic, and not from a default. Silently inferring "they said a brand, and today's results happen to be phones, so they must mean phones" is how a request becomes an answer about something else. Inference is only legitimate when it resolves a word the user actually used against the tenant's own published data; anything beyond that is a guess and must become a question. A wrong assumption is worse than one extra turn, because the customer cannot see what was assumed.
...
* **Always Boot And Test Locally — Never Against Public/Deployed Servers:** Every change must be verified against a freshly booted local system built from the current source, not only against unit tests. Boot the local AI Hub and the local AI-KART website on the current checkout (on isolated ports if the default ports hold an older process), confirm the current source markers are being served, exercise the affected flow end to end, and record the local run in `handoff.md`. Do **not** test, drive, or measure behavior against public or deployed servers (for example `demo1.ergobite.com` or any `*.vercel.app` origin); when a local site is wired to a public Hub for convenience, repoint it to the local Hub for the test and revert afterwards. A green unit suite is necessary but not sufficient; a defect is not considered fixed until the running local system demonstrates the corrected behavior. If the local system genuinely cannot boot in the environment, record the exact blocker and evidence in `handoff.md` rather than declaring the work complete.
* **Report Honestly:** A timed-out, skipped, unavailable, flaky, or unrun check is not a pass. Record its exact state in `handoff.md`.
* **Standard Local Voice Test Procedure:** Drive real turns with synthesized speech rather than typed text, using these exact steps. Do not invent a new mechanism each time.
  1. Boot the local stack: AI-KART API, the AI-KART frontend, and the Hub built from the current source. Rebuild the Hub image when its source is baked in rather than mounted, or the test silently exercises stale code.
  2. Edit `Vercel_website/frontend/index.html` to point the installer at the local Hub (`http://127.0.0.1:<hub-port>/install.js?site_id=<site>`). This is a temporary edit; do not attempt runtime injection, which a page reload discards.
  3. Generate one WAV per question with Windows SAPI at 16 kHz mono, and serve them from a temporary local file server.
  4. In the browser, replace `MediaRecorder` with a shim that hands the widget the WAV where the microphone buffer would go, and `getUserMedia` with a silent stream so permission handling is unchanged. Drive the orb, then read the spoken answer, the resulting URL, and the rendered DOM.
  5. Cover a mix of turns in one session: ordinary requests, navigation, an item the catalog does not carry, an absurd request, and at least two genuinely ambiguous ones that must produce a clarifying question rather than a guess.
  6. **Capture a screenshot after every turn, full page as well as viewport.** Reading the DOM proves what exists; only a screenshot proves what the customer can actually see. Panel content that is empty, clipped, invisible against its background, or scrolled out of view passes a DOM assertion and fails a human. A viewport shot alone hides everything below the fold, which is where a result list disagrees with what was said - so take a full-height capture of the whole page too, and compare the records named aloud against the records actually rendered. Attach the screenshots, or their paths, to `handoff.md`.
  7. Restore `index.html` to the deployment installer URL, stop the temporary server, and delete the generated audio. Record the transcript, URLs, and counts in `handoff.md`.
