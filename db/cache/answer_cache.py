...
def lookup_answer_cache(site_id: str, question: str, session_id: str = "") -> dict[str, Any] | None:
    ...
    exact = _exact_cache_match(site_id, safe_session_id, normalized, data_version)
    if exact:
    ...
    with get_db(site_id) as conn:
        row = conn.execute(
            ...
            INSERT INTO answer_cache
                (
                    session_id, normalized_question, question, question_embedding, answer_text,
                    answer_scope, cache_type, source_ids_json, source_urls_json,
                    ui_actions_json, confidence, data_version, expires_at
                )
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (session_id, normalized_question, data_version) DO UPDATE SET
...
def _exact_cache_match(
    ...
    with get_db(site_id) as conn:
        row = conn.execute(
            ...
            LIMIT 1
            """,
            (session_id, normalized, data_version),
        ).fetchone()
    return dict(row) if row else None


def _semantic_cache_match(
    site_id: str,
    session_id: str,
    question: str,
    data_version: int,
    ...
    with get_db(site_id) as conn:
        row = conn.execute(
            ...
            LIMIT 1
            """,
            (embedding, session_id, data_version, embedding),
        ).fetchone()
    if not row:
        return None
...
def normalize_session_id(value: Any) -> str:
    return str(value or "").strip()[:MAX_CACHE_SESSION_CHARS]


def _embed_question(question: str) -> Any:
    from agent.retrieval.product_rag import _embed
