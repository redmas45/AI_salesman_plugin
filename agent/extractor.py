...
Runs only when LLM_EXTRACTOR_ENABLED=true and OPENAI_API_KEY is configured.
...
def extract_selectors_from_html(
    ...
    if not config.OPENAI_API_KEY:
        logger.info("LLM extractor skipped: no OPENAI_API_KEY configured.")
        return None
    ...
    try:
        ...
        logger.warning("LLM extractor OpenAI call failed for %s: %s", site_id, exc)
        return None
