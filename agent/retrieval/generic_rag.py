...
import config
from agent.rag import _embed, _clean_query_for_fts, _build_tsquery
...
def _vectorize_missing_knowledge_batch(site_id: str) -> int:
    ...
    try:
        from db.database import rebuild_knowledge_search_vectors
...
def _fuzzy_knowledge_search(
    ...
    try:
        ...
        if entity_types:
            type_clause = "AND entity_type = ANY(%s)"
            params.append(entity_types)
        params.extend([clean, 0.15, clean, limit])
        with get_db(site_id) as conn:
            rows = conn.execute(
                f"""
                SELECT *,
