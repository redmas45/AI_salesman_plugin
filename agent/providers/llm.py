...
import config
from agent.prompts.context_budget import build_context_messages
from agent.prompts.ecommerce_prompt import build_system_prompt, format_products_for_prompt
from agent.prompts.page_context import format_page_context, sanitize_page_context
from agent.prompts.generic import build_generic_system_prompt, format_knowledge_for_prompt
from agent.providers.provider_status import record_provider_failure, record_provider_success
...
def _call_llm(system_prompt: str, messages: list[dict[str, Any]]) -> str:
    ...
    logger.info("LLM | Azure deployment=%s", config.AZURE_OPENAI_CHAT_DEPLOYMENT)
...
def generate_response(
    ...
    try:
        ...
        record_provider_success("azure_openai")
        logger.info(
        ...
        if _is_quota_exhausted_error(exc):
            record_provider_failure("azure_openai", exc, category="quota_exhausted")
            return QUOTA_EXHAUSTED_RESPONSE.copy()
        record_provider_failure("azure_openai", exc, category="error")
